...
def elementos_exclusivos(s: list, t: list) -> list:
    ...
    for i in range(len(s)):
      if not pertenece(s[i],t):
         if not pertenece(s[i],exclusivo): 
                exclusivo.append(s[i])
      else:
                exclusivo = exclusivo
    for g in range(len(t)):
        if not pertenece(t[g],s):
          if not pertenece(t[g],exclusivo): 
                exclusivo.append(t[g])
        else:
                exclusivo = exclusivo
    return exclusivo

def pertenece(s,list)-> bool:
    if s in list:
        return True
    else: 
        return False
...
def contar_traducciones_iguales(ingles: dict, aleman: dict) -> int:
    ...
    for key,value in ingles.items():
         for key2,value2 in aleman.items():
